...
const AddShows = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!selectedMovie || Object.keys(dateTimeSelection).length === 0 || !showPrice) {
        ...
        setAddingShow(false); // Make sure to stop loading on validation error
        return;
      }
      ...
      // FIX #1: Changed '/api/show/add' to '/api/shows/add'
  ...
  useEffect(() => {
    ...
    // FIX #2: Added 'user' to dependency array
  }, [user]);

  // FIX #3: Changed 'setNowPlayingMovies.length' to 'nowPlayingMovies.length'
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      <button onClick={handleSubmit} disabled={addingShow} className='bg-primary text-white px-8 py-2 mt-6 rounded
        hover:bg-primary/90 transition-all cursor-pointer disabled:bg-gray-500'>
